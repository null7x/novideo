...
def _build_tiktok_filter_v2(width: int, height: int, duration: float, target_fps: float = 30) -> Tuple[str, str, dict]:
    ...
    # Используем on (frame number) вместо t для zoompan
    total_frames = int(duration * 30)
    motion_type = random.choice(["zoom_in", "zoom_out", "zoom_pulse"])
    if motion_type == "zoom_in":
        zoom_end = _rand(1.03, 1.06)
        z_expr = f"min(1+on*{(zoom_end-1)/total_frames},{zoom_end})"
        filters.append(f"zoompan=z='{z_expr}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s={width}x{height}:fps=30")
    elif motion_type == "zoom_out":
        zoom_start = _rand(1.04, 1.07)
        z_expr = f"max({zoom_start}-on*{(zoom_start-1)/total_frames},1)"
        filters.append(f"zoompan=z='{z_expr}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s={width}x{height}:fps=30")
    else:  # zoom_pulse
        zoom_delta = _rand(0.02, 0.04)
        z_expr = f"1+{zoom_delta}*sin(on/{total_frames}*3.14*4)"
        filters.append(f"zoompan=z='{z_expr}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s={width}x{height}:fps=30")
