...
async def cmd_vip(message: Message):
    """ /vip user_id — выдать VIP пользователю """
    ...
    await message.answer(TEXTS.get("vip_granted", "✅ VIP выдан").format(user_id=target_id))

@dp.message(Command("premium"))
async def cmd_premium(message: Message):
    """ /premium user_id — выдать Premium пользователю """
    if message.from_user.id not in ADMIN_IDS:
        await message.answer(TEXTS.get("not_admin", "⛔ Нет доступа"))
        return
    ...
    await message.answer(TEXTS.get("premium_granted", "✅ Premium выдан").format(user_id=target_id))

@dp.message(Command("removeplan"))
async def cmd_removeplan(message: Message):
    """ /removeplan user_id — убрать план (сделать free) """
    if message.from_user.id not in ADMIN_IDS:
        await message.answer(TEXTS.get("not_admin", "⛔ Нет доступа"))
        return
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.answer("Использование: /removeplan user_id")
        return
