...
def _build_forced_hook(width: int, height: int, duration: float) -> str:
    ...
    # Экранируем двоеточие в тексте для FFmpeg
    safe_text = hook_text.replace(":", r"\:")
    hook_filter = (
...
def _build_tiktok_filter_v2(width: int, height: int, duration: float) -> Tuple[str, str, dict]:
    ...
    if motion_type == "zoom_in":
        ...
        z_expr = f"min({zoom_start}+on*{(zoom_end-zoom_start)/30/duration},{zoom_end})"
        filters.append(f"zoompan=z={z_expr}:x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):d=1:s={width}x{height}:fps=30")
    elif motion_type == "zoom_out":
        zoom_start = _rand(1.04, 1.08)
        zoom_end = 1.0
        z_expr = f"max({zoom_start}-on*{(zoom_start-zoom_end)/30/duration},{zoom_end})"
        filters.append(f"zoompan=z={z_expr}:x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):d=1:s={width}x{height}:fps=30")
    elif motion_type == "pan":
        pan_x = random.randint(-20, 20)
        pan_y = random.randint(-10, 10)
        crop_x = f"20+{pan_x}*t/{duration}"
        crop_y = f"10+{pan_y}*sin(t/{duration}*3.14)"
        filters.append(f"crop=w={width-40}:h={height-20}:x={crop_x}:y={crop_y},scale={width}:{height}:flags=lanczos")
    else:  # combo: zoom + pan
        zoom_delta = _rand(0.02, 0.05)
        pan_x = random.randint(-10, 10)
        z_expr = f"1+{zoom_delta}*sin(t/{duration}*3.14)"
        x_expr = f"iw/2-(iw/zoom/2)+{pan_x}*sin(t*2)"
        filters.append(f"zoompan=z={z_expr}:x={x_expr}:y=ih/2-(ih/zoom/2):d=1:s={width}x{height}:fps=30")
    
    # 3b. MICRO-SHAKE для "живости" (всегда)
    shake_intensity = random.randint(2, 4)
    shake_freq = _rand(10, 18)
    shake_x_expr = f"{shake_intensity}+{shake_intensity}*sin({shake_freq}*t)"
    shake_y_expr = f"{shake_intensity}+{shake_intensity}*sin({shake_freq}*t*1.3)"
    filters.append(
...
def _build_youtube_filter_v2(width: int, height: int, duration: float) -> Tuple[str, str, dict]:
    ...
    if motion_type == "zoom_in":
        z_expr = f"min(1+on*{zoom_delta/30/duration},{1+zoom_delta})"
        filters.append(f"zoompan=z={z_expr}:x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):d=1:s={width}x{height}:fps=30")
    elif motion_type == "zoom_out":
        z_expr = f"max({1+zoom_delta}-on*{zoom_delta/30/duration},1)"
        filters.append(f"zoompan=z={z_expr}:x=iw/2-(iw/zoom/2):y=ih/2-(ih/zoom/2):d=1:s={width}x{height}:fps=30")
    else:
        pan_x = random.randint(-15, 15)
        crop_x = f"15+{pan_x}*t/{duration}"
        filters.append(f"crop=w={width-30}:h={height}:x={crop_x}:y=0,scale={width}:{height}:flags=lanczos")
    
    # Micro-shake
    shake = random.randint(2, 3)
    shake_freq = _rand(10, 15)
    shake_x_expr = f"{shake}+{shake}*sin({shake_freq}*t)"
    shake_y_expr = f"{shake}+{shake}*sin({shake_freq}*t*1.2)"
    filters.append(
