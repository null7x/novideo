...
def _build_youtube_filter_v2(width: int, height: int, duration: float, target_fps: float = 30) -> Tuple[str, str, dict]:
    ...
    if motion_type == "zoom_in":
        z_expr = f"min(1+on*{zoom_delta/total_frames},{1+zoom_delta})"
        filters.append(f"zoompan=z='{z_expr}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s={width}x{height}:fps=30")
    elif motion_type == "zoom_out":
        z_expr = f"max({1+zoom_delta}-on*{zoom_delta/total_frames},1)"
        filters.append(f"zoompan=z='{z_expr}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s={width}x{height}:fps=30")
    else:  # zoom_pulse
        z_expr = f"1+{zoom_delta/2}*sin(on/{total_frames}*3.14*4)"
        filters.append(f"zoompan=z='{z_expr}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s={width}x{height}:fps=30")
    
    # Micro-shake - упрощённый статический crop
