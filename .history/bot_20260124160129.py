...
async def download_youtube_video(url: str, output_path: str) -> bool:
    ...
    try:
        ...
        # Публичные API для скачивания YouTube
        api_endpoints = [
            f"https://api.vevioz.com/api/button/videos?url=https://www.youtube.com/watch?v={video_id}",
            f"https://yt1s.com/api/ajaxSearch/index?q=https://www.youtube.com/watch?v={video_id}",
        ]
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Пробуем cobalt.tools API (работает стабильно)
            try:
                ...
                async with session.post(cobalt_url, json=cobalt_data, headers=cobalt_headers, timeout=20) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        if data.get('status') == 'stream' or data.get('status') == 'redirect':
                            video_url = data.get('url')
                            logger.info(f"[YouTube] Cobalt API success")
            except Exception as e:
                logger.warning(f"[YouTube] Cobalt API failed: {e}")
            
            # Пробуем другой API если cobalt не сработал
            if not video_url:
                try:
                    # ssyoutube API
                    ss_url = f"https://ssyoutube.com/api/convert"
                    ss_data = {"url": f"https://www.youtube.com/watch?v={video_id}"}
                    async with session.post(ss_url, data=ss_data, headers=headers, timeout=15) as resp:
                        if resp.status == 200:
                            data = await resp.json()
                            if 'url' in data:
                                video_url = data['url']
                                logger.info(f"[YouTube] ssyoutube API success")
                except Exception as e:
                    logger.warning(f"[YouTube] ssyoutube API failed: {e}")
            ...
            async with session.get(video_url, headers=headers, timeout=180) as video_resp:
