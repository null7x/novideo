...
def test_v290():
    ...
    if len(EFFECT_TEMPLATES) >= 4:
        ...
        passed += 1
        for tid in ["viral_tiktok", "clean_youtube", "reels_style", "shorts_format"]:
    ...
    # Test new texts
    new_text_keys = [
        "batch_start", "batch_progress", "batch_done", "batch_limit",
        "trim_usage", "trim_invalid", "trim_processing", "trim_done", "trim_set",
        "trim_help", "trim_cleared", "trim_invalid_format",
        "music_usage", "music_waiting", "music_processing", "music_done",
        "music_help", "music_received",
        "convert_menu", "convert_processing", "convert_done", "convert_help",
        "watermark_usage", "watermark_set", "watermark_removed", "watermark_help",
        "resolution_menu", "resolution_changed", "resolution_select",
        "templates_menu", "template_applied", "templates_select",
        "reminder_set", "reminder_notify", "reminder_help",
        "achievement_unlocked", "achievements_list", "level_up", "profile", "leaderboard",
    ...
    print(f"  ✅ New texts: {found_ru}/{len(new_text_keys)} RU, {found_en}/{len(new_text_keys)} EN")
    
    # Test new buttons
    new_btn_keys = [
        "trim", "add_music", "convert", "watermark", "resolution",
        "templates", "reminder", "achievements", "profile", "leaderboard", "analytics",
        "to_gif", "to_mp3", "to_webm", "1080p", "720p", "480p", "original",
        "position_tl", "position_tr", "position_bl", "position_br",
    ]
    ...
    print(f"  ✅ New buttons: {len(new_btn_keys)} keys checked")
