...
def get_settings_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if plan == "free":
        buttons.append([InlineKeyboardButton(text=BUTTONS["buy_premium"], callback_data="buy_premium")])
    
    buttons.append([InlineKeyboardButton(text=BUTTONS["back"], callback_data="back_to_start")])
...
async def cmd_buy(message: Message):
    ...
    await message.answer(TEXTS["buy_premium"], reply_markup=get_buy_premium_keyboard())
...
async def cb_process(callback: CallbackQuery):
    ...
    if not can_process:
        if reason == "soft_block":
            ...
            stats = rate_limiter.get_stats(user_id)
            await callback.answer(
                TEXTS.get("monthly_limit_reached", "⚠️ Лимит на 30 дней исчерпан ({used}/{limit})").format(
...
async def download_video_from_url(url: str, output_path: str) -> bool:
    """Скачать видео по ссылке используя yt-dlp или специальные методы"""
    try:
...
async def download_kuaishou_video(url: str, output_path: str) -> bool:
    """Скачать видео из Kuaishou через парсинг страницы"""
    try:
        import aiohttp
        ...
        if success and result_path:
            try:
                ...
                await bot.send_video(
                    ...
                    video=video_file,
                    caption=TEXTS["done"],
                    reply_markup=get_result_keyboard(new_short_id)
                )
