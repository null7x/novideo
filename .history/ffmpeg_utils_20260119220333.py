...
def _build_forced_hook(width: int, height: int, duration: float) -> str:
    ...
    # Alpha с экранированными запятыми для FFmpeg
    alpha = f"if(lt(t\\,{fade_in})\\,t/{fade_in}\\,if(lt(t\\,{fade_in + hold_time})\\,1\\,if(lt(t\\,{hook_duration})\\,({hook_duration}-t)/{fade_out}\\,0)))"
...
def _build_tiktok_filter_v2(width: int, height: int, duration: float) -> Tuple[str, str, dict]:
    ...
    # Простой fade через встроенный fade filter
    filters.append(
        f"drawtext=text='{hook_text}':"
    ...
    if random.random() > 0.4:
        ...
        # Простой enable без сложного alpha
        filters.append(
...
def _build_youtube_filter_v2(width: int, height: int, duration: float) -> Tuple[str, str, dict]:
    ...
    hook_duration = min(2.0, duration * 0.25)
    # Простой drawtext без сложного alpha
    filters.append(
        f"drawtext=text='{hook_text}':"
        f"fontsize={hook_fontsize}:"
        f"fontcolor=white:"
