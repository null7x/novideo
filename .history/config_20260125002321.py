...
class PlanLimits:
    ...
    priority: int = 0                 # 0=low, 1=medium, 2=high (queue priority)
    can_disable_text: bool = False    # Может отключать текст
    quality_options: list = None      # Доступные качества
    max_resolution: str = "1080p"     # Максимальное разрешение
    templates_count: int = 7          # Доступных шаблонов
    anti_reupload_level: str = "low"  # low/medium/hardcore
    has_auto_unique: bool = False     # Автоуникализация
    has_watermark_trap: bool = False  # Цифровой отпечаток
    has_project_history: bool = False # История проектов
    history_days: int = 0             # Дней хранения истории
    queue_priority: str = "slow"      # slow/priority/instant

# ══════════════════════════════════════════════════════════════════════════════
# 💎 PRICING MODEL v3.2.0 — VIREX PRO
# ══════════════════════════════════════════════════════════════════════════════
# 🆓 FREE:    $0       - попробовать (2 видео/день, боль)
# 💠 VIP:     $4.99/мес - рабочий инструмент (15 видео/день)
# 👑 PREMIUM: $9.99/мес - безлимит для фармеров/студий
# ══════════════════════════════════════════════════════════════════════════════

PLAN_PRICES = {
    "vip": {
        "1_month": {"price": 4.99, "label": "$4.99", "days": 30},
        "3_months": {"price": 12.99, "label": "$12.99", "days": 90, "save": "13%"},
        "1_year": {"price": 44.99, "label": "$44.99", "days": 365, "save": "25%", "popular": True},
    },
    "premium": {
        "1_month": {"price": 9.99, "label": "$9.99", "days": 30},
        "3_months": {"price": 24.99, "label": "$24.99", "days": 90, "save": "17%"},
        "1_year": {"price": 79.99, "label": "$79.99", "days": 365, "save": "33%", "popular": True},
    },
}

# 🔥 Pay-as-you-go (донат модель для сомневающихся)
PAY_AS_YOU_GO = {
    "pack_10": {"videos": 10, "price": 1.00, "label": "+10 видео за $1"},
    "pack_50": {"videos": 50, "price": 4.00, "label": "+50 видео за $4", "popular": True},
    "pack_100": {"videos": 100, "price": 7.00, "label": "+100 видео за $7", "save": "30%"},
}

# 🎁 Промо-логика
PROMO_CONFIG = {
    "first_vip_discount": 0.50,       # -50% на первый VIP
    "referral_premium_hours": 24,     # 24 часа Premium за приглашение
    "referral_bonus_videos": 2,       # +2 видео за каждого реферала (VIP)
    "achievement_bonus_videos": 1,    # +1 видео за достижение
    "vip_to_premium_discount": 0.20,  # -20% на Premium для VIP
}

# 🎯 Anti-Abuse защита
ANTI_ABUSE_CONFIG = {
    "free_delay_seconds": 60,         # Задержка между обработками для Free
    "free_daily_limit": 2,            # Жёсткий лимит Free
    "free_templates_random": True,    # Случайный лимит шаблонов для Free
    "ip_limit_per_day": 5,            # Лимит Free аккаунтов с одного IP
    "fingerprint_check": True,        # Проверка device fingerprint
    "suspicious_activity_block": 300, # Блок на 5 мин при подозрительной активности
}

# Ограничения по разрешению
RESOLUTION_LIMITS = {
    "free": {"max_width": 1920, "max_height": 1080, "label": "1080p"},
    "vip": {"max_width": 3840, "max_height": 2160, "label": "4K"},
    "premium": {"max_width": 7680, "max_height": 4320, "label": "8K"},
}

# Доступные шаблоны по планам
TEMPLATE_ACCESS = {
    "free": ["none", "viral", "bright", "vintage", "smooth", "aesthetic", "summer", "chill"],  # 7 базовых
    "vip": "all_non_premium",  # Все кроме premium-only (25+)
    "premium": "all",          # Все 40+ шаблонов
}

PLAN_LIMITS: Dict[str, PlanLimits] = {
    "free": PlanLimits(
        videos_per_day=2,             # 2 видео в день — показать ценность
        videos_per_week=14,           # ~14 в неделю (теоретически)
        cooldown_seconds=60,          # Задержка 60 сек — создаёт боль
        max_file_size_mb=50,
        priority=0,                   # Низкий приоритет в очереди
        can_disable_text=False,
        quality_options=["low", "medium"],
        max_resolution="1080p",
        templates_count=7,            # Только базовые шаблоны
        anti_reupload_level="low",    # Только Low защита
        has_auto_unique=False,        # ❌ Нет авто-режима
        has_watermark_trap=False,     # ❌ Нет watermark trap
        has_project_history=False,    # ❌ Нет истории
        history_days=0,
        queue_priority="slow",        # Медленная очередь
    ),
    "vip": PlanLimits(
        videos_per_day=15,            # 15 видео в день — рабочий инструмент
        videos_per_week=100,          # 100 видео в неделю
        cooldown_seconds=10,          # Минимальная задержка
        max_file_size_mb=100,
        priority=1,                   # Приоритет в очереди
        can_disable_text=True,
        quality_options=["low", "medium", "max"],
        max_resolution="4K",          # До 4K
        templates_count=25,           # 25+ шаблонов
        anti_reupload_level="medium", # Medium защита
        has_auto_unique=False,        # ❌ Нет авто-режима (только Premium)
        has_watermark_trap=False,     # ❌ Нет watermark trap
        has_project_history=True,     # ✅ История проектов
        history_days=7,               # 7 дней
        queue_priority="priority",    # Приоритетная очередь
    ),
    "premium": PlanLimits(
        videos_per_day=999999,        # ♾ Безлимит
        videos_per_week=999999,       # ♾ Безлимит
        cooldown_seconds=0,           # Без задержки
        max_file_size_mb=100,
        priority=2,                   # Максимальный приоритет
        can_disable_text=True,
        quality_options=["low", "medium", "max"],
        max_resolution="8K",          # До 8K
        templates_count=999,          # Все 40+ шаблонов
        anti_reupload_level="hardcore", # Hardcore защита
        has_auto_unique=True,         # ✅ Auto-Unique Mode
        has_watermark_trap=True,      # ✅ Watermark-Trap
        has_project_history=True,     # ✅ История проектов
        history_days=30,              # 30 дней
        queue_priority="instant",     # Мгновенный рендер
    ),
}

# Бонусные видео (pay-as-you-go баланс)
# Хранится в user.bonus_videos
